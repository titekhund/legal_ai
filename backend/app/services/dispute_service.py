...
шдм юар сахарзедкнс савагасаюагн гаедбис дхсодрти.

шдми алнъамаа:
1. ваамакижн лньнгдбуки фимамсза салимистрнс гаедбис вагаьчедтикдбдби
2. оасуюи васъд йизюеас ал вагаьчедтикдбдбжд гачргмнбиз
3. лиузизн йнмйрдтуки сахлддби га лази рдкдеамтури маьикдби
4. гаайаеширн вагаьчедтикдбдби савагасаюагн йнгдхсис луюкдбзам
...
оасуюи умга шдиъаегдс:
- оиргаоир оасуюс йизюеажд
- лизиздбас йнмйрдтук сахлддбжд (гнйулдмтис #, зарици)
- йаеширс савагасаюагн йнгдхсис луюкдбзам
"""
...
class DisputeFilters(BaseModel):
    ...
    court: Optional[str] = Field(
        None,
        description="Court name (e.g., 'сайнмституъин', 'уждмадси')"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "court": "уждмадси сасаларзкн",
                "date": "2023-05-15",
                "summary": "сасаларзкнл вамиюика гцв-с гаеа...",
...
class DisputeResponse(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "answer": "гцв-с вамайедзи арис 18%...",
                "cases_cited": [
                    {
                        "case_id": "001",
                        "court": "уждмадси сасаларзкн",
...
class DisputeService:
    ...
    async def query(
        ...
        if not filtered_results:
            ...
            return DisputeResponse(
                answer="едр лниыдбма рдкдеамтури сахлддби зхедми йизюеисзеис.",
                cases_cited=[],
    ...
    def _convert_to_dispute_cases(
        ...
        for result in search_results:
            ...
            case = DisputeCase(
                ...
                court=metadata.get("court", "уъмнби сасаларзкн"),
                date=case_date,
    ...
    def _build_context(self, cases: List[DisputeCase]) -> str:
        ...
        for i, case in enumerate(cases, 1):
            context_parts.append(
                f"сахлд #{i} (ID: {case.case_id})\n"
                f"сасаларзкн: {case.court}\n"
                f"зарици: {case.date}\n"
                f"ъитирдбуки луюкдби: {', '.join(case.cited_articles) if case.cited_articles else 'ар арис лизиздбуки'}\n"
                f"шимаарси: {case.summary}\n"
            )
    ...
    async def _generate_answer(
        ...
        try:
            ...
            return "едр лнюдрюга оасуюис вдмдраъиа. взюнез съагнз лнвеиамдбиз.", "error"

    def _extract_tax_articles(self, text: str) -> List[str]:
        """Extract tax code article numbers from text"""
        # Pattern for Georgian article references: луюки 123, луюки 168.1, etc.
        pattern = r'луюки\s+(\d+(?:\.\d+)?(?:\.[а-№])?)'
        matches = re.findall(pattern, text)

        # Also look for bare numbers that might be articles
        # Pattern for "article 123" style
        pattern2 = r'(?:article|луюки|AB\.)\s*(\d+)'
